...
orderController.post('/userOrders', async (req, res) => {
    const userId = req.user._id;
    console.log('Request Body:', req.body);  // Add more logging to confirm shippingDetails
    try {
      const order = await orderService.createOrder(userId, req.body.shippingDetails);
...
orderController.patch('/approve/:id', async (req, res) => {
  const { id } = req.params;
  const userRole = req.user.role;
...
orderController.patch('/reject/:id', async (req, res) => {
  const { id } = req.params;
  const userRole = req.user.role;
